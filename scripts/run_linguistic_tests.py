...
from pipeline.hybrid_v2 import HybridPipelineV2  # <-- importa V2
...
def evaluate_adapter(adapter, cases):
    ...
    for case in cases:
        ...
        # Se o adapter tiver método predict (Deterministic, Tucano), usa ele
        if hasattr(adapter, "predict"):
            prediction = adapter.predict(msg)
        else:
            # Para pipelines (Hybrid), usa process sem estado
            output = adapter.process(msg, None)
...
def main():
    ...
    adapters = {
        ...
        "Hybrid-V2": HybridPipelineV2()  # <-- usa V2
    }
