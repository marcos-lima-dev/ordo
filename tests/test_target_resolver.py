...
def test_last_item_does_not_authorize_target(state_three_items):
    """Last item sozinho NÃO autoriza target."""
    resolver = TargetResolver()
...
def test_confirm_order_does_not_require_target(state_two_items):
    """CONFIRM_ORDER não precisa de target; o resolver deve retornar UNKNOWN sem quebrar."""
    resolver = TargetResolver()
    result = resolver.resolve("pode fechar", state_two_items)
    assert result.status == TargetStatus.UNKNOWN

def test_cancel_order_does_not_require_target(state_two_items):
    """CANCEL_ORDER não precisa de target; o resolver deve retornar UNKNOWN sem quebrar."""
    resolver = TargetResolver()
