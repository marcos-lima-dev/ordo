...
@register_adapter("gliner")
class GLiNERAdapter(CandidateAdapter):
    def __init__(self, model_name="urchade/gliner_medium", device="cpu"):
        self.model_name = model_name
        self.device = device
        print(f"Carregando modelo {model_name}...")
        self.model = GLiNER.from_pretrained(model_name)
        # Labels em português para extração
    ...
    def predict(self, message: str) -> dict:
        ...
        # Fallback para intenção (regras simples)
        if intent == "UNKNOWN":
